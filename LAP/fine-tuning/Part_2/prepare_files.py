...
CWD='/home/hmahdi/caffe/examples/imdb/Smaller_Pre-training_distribution/'
...
portions=[[0.00125,32],[0.0025,32],[0.00375,32]]#,[0.005,32],[0.0065,32]]
stepsize=["3","7","10"]#,"13","16"]
max_iter=["15","35","50"]#,"65","80"]
...
for p in portions:
	for index in xrange(0,p[1]):
		...
		Network_address='Learning_curve_prototxts/DEX_'+str(p[0])+'_'+str(index)+'.prototxt'
		with open(Network_address, 'w') as file:
  			file.write(filedata)	

		#Create the solvers
		Network_address=CWD+Network_address
		Solver_address='Learning_curve_prototxts/DEX_solver_'+str(p[0])+'_'+str(index)+'.prototxt'
		...
		solver_text.write("snapshot_prefix: \"/srv/glusterfs/hmahdi/IMDB/learning_curve_snapshots/Fine_tune_"+str(p[0])+'_'+str(index)+"\"\n")
